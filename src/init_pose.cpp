...
static std::string print_elem(const cv::Mat & m, const int row, const int col)
{
  std::stringstream ss;
  switch (m.channels()) {
    case 1:
    ...
    case 3:
      ss << "[" << m.at<cv::Vec3d>(row, col)[0] << ", "
         << m.at<cv::Vec3d>(row, col)[1] << ", " << m.at<cv::Vec3d>(row, col)[2]
         << "]";
      break;
    default:
...
#ifdef DEBUG_POSE
static void test_reprojection(
  const cv::Mat & ip, const cv::Mat & wp, const cv::Mat & R,
  const cv::Mat & tvec)
{
  cv::Mat rwp = (R * wp.t()).t();
...
static void decompose_homography_simple(
  ...
  const cv::Mat & H, cv::Mat * R, cv::Mat * rvec, cv::Mat * tvec)
{
  ...
  cv::Mat U1(3, 3, CV_64F), U2(3, 3, CV_64F), W1(3, 3, CV_64F),
    W2(3, 3, CV_64F);
  ...
  bool use1 = hasGoodR1 &&
              (fabs(N1.at<double>(2)) > fabs(N2.at<double>(2)) || !hasGoodR2);
  *R = use1 ? R1 : R2;
  cv::Mat N = use1 ? N1 : N2;
...
static void decompose_homography_svd(
  const cv::Mat & H, cv::Mat * R, cv::Mat * rvec, cv::Mat * tvec)
{
  //
...
static cv::Mat undistort_points(
  const cv::Mat & im, const cv::Mat & K, DistortionModel distModel,
  const cv::Mat & D)
{
...
static bool find_pose_by_homography(
  const cv::Mat & wp, const cv::Mat & ip, cv::Mat * rvec, cv::Mat * tvec)
{
  cv::Mat wc = wp.clone();
  ...
  DistortionModel distModel, const cv::Mat & D, const Params & params)
{
  ...
  if (std::abs(beta_orig - beta_min) > 0.1 && qr < params.maxAmbiguityRatio) {
    // warn if the original homography angle is not the actual
    // minimum, and the new minimum has significantly lower error
    LOG_WARN(
      "original rot: " << beta_orig << " is off from min: " << beta_min
                       << " view angle: " << view_angle * RAD_2_DEG
                       << ", qr: " << qr);
  }
