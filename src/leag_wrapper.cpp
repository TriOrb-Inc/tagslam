...
struct Tracker::Impl {
  ...
  // ここに vendor クラスを保持
  // 例：leag::LentiMarkTracker tracker;
  // 実際の型名に合わせて修正してください
  // もしポインタ生成が必要なら unique_ptr でもOK
};
...
void Tracker::initializeLeagDetectors(std::vector<std::string> camera_topics, std::string str_camfile)
{     
  ...
  impl_->LMT_list_.reserve(camera_topics.size());
  for (const auto & cam : camera_topics) {
    ...
    const int res2 = lmt.setMarkerParams(str_mkfile.c_str());
    std::cout << "cam topic: " << cam << " setCamParams=" << res1 << " setMarkerParams=" << res2 << std::endl;
    impl_->LMT_list_.push_back(lmt);
  }
}
...
bool Tracker::detect_marker(const cv::Mat & img, ApriltagArray::SharedPtr tags, uint8_t camera_index)
{
  ...
  int res;
  if ((res = impl_->LMT_list_[camera_index].detect_nonAR(img, ids, tagslam_corners)) < 0) {
    std::cerr << "LEAG detect error for camera index " << camera_index << ": " << res << std::endl;
    return false;
  }

  std::vector<leag::LentiMarkTracker::ResultData> m_data;
  int candidateNum = impl_->LMT_list_[camera_index].getResult(m_data);
  if (candidateNum < 0) {
    return false;
  }
  ...
  candidateNum = impl_->LMT_list_[camera_index].getResultPKGData(ids_cor, leag_corners);
  candidateNum = impl_->LMT_list_[camera_index].getResultPKGCenterData(ids_cen, centers);
  if (candidateNum < 0) {
    return false;
  }
